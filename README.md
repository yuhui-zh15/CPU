...
| 序号   | 测试程序                      | 功能测试点                                    | 结果   |
| :--- | :------------------------ | :--------------------------------------- | ---- |
| 1    | ADD                       | 执行 ADD 指令是否产生正确的运算结果（未测试整型溢出例外的情况）       | P    |
| 2    | ADDI                      | 执行 ADDI 指令是否产生正确的运算结果（未测试整型溢出例外的情况）      | P    |
| 3    | ADDU                      | 执行 ADDU 指令是否产生正确的运算结果                    | P    |
| 4    | ADDIU                     | 执行 ADDIU 指令是否产生正确的运算结果                   | P    |
| 5    | SUB                       | 执行 SUB 指令是否产生正确的运算结果（未测试整型溢出例外的情况）       | P    |
...
| 14   | MULTU                     | 执行 MULTU 指令是否产生正确的运算结果                   | P    |
...
| 32   | BGTZ                      | 执行 BGTZ 指令是否产生正确的判断和跳转结果（延迟槽指令为 nop，未测试延迟槽） | P    |
| 33   | BLEZ                      | 执行 BLEZ 指令是否产生正确的判断和跳转结果（延迟槽指令为 nop，未测试延迟槽） | P    |
| 34   | BLTZ                      | 执行 BLTZ 指令是否产生正确的判断和跳转结果（延迟槽指令为 nop，未测试延迟槽） | P    |
| 35   | BGEZAL                    | 执行 BGEZAL 指令是否产生正确的判断、跳转和链接结果（延迟槽指令为 nop，未测试延迟槽） | N    |
...
| 49   | LH                        | 结合 SW 指令，执行 LH 指令是否产生正确的访存结果             | P    |
| 50   | LHU                       | 结合 SW 指令，执行 LHU 指令是否产生正确的访存结果            | P    |
| 51   | LW                        | 结合 SW 指令，执行 LW 指令是否产生正确的访存结果             | P    |
| 52   | SB                        | 结合 LW 指令，执行 SB 指令是否产生正确的访存结果             | P    |
| 53   | SH                        | 结合 LW 指令，执行 SH 指令是否产生正确的访存结果             | P    |
...
| 58   | ADD\_EX                   | 测试 ADD 指令整型溢出例外                          | P    |
| 59   | ADDI\_EX                  | 测试 ADDI 指令整型溢出例外                         | P    |
| 60   | SUB\_EX                   | 测试 SUB 指令整型溢出例外                          | P    |
