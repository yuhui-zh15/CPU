...
| 序号   | 测试程序                      | 功能测试点                                    | 仿真结果 | 11MHz时钟 | 50MHz时钟  |
| :--- | :------------------------ | :--------------------------------------- | ---- | ------- | -------- |
...
| 5    | SUB                       | 执行 SUB 指令是否产生正确的运算结果（未测试整型溢出例外的情况）       | P    | P       | N        |
...
| 13   | MULT                      | 执行 MULT 指令是否产生正确的运算结果                    | P    | P       | N        |
| 14   | MULTU                     | 执行 MULTU 指令是否产生正确的运算结果                   | P    | P       | N        |
...
| 28   | SRL                       | 执行 SRL 指令是否产生正确的移位结果                     | P    | P       | 1A       |
...
| 40   | JALR                      | 执行 JALR 指令是否产生正确的跳转和链接结果（延迟槽指令为 nop，未测试延迟槽） | P    | P       | ALLP     |
...
| 44   | MTLO                      | 执行 MTLO 指令是否正确地将寄存器值写入 HI 寄存器，执行MFLO 指令是否正确地读出 HI 寄存器的值到寄存器 | P    | P       | 28       |
| 45   | BREAK                     | 执行 BREAK 指令是否正确地产生断点例外                   | N    | N       |          |
| 46   | SYSCALL                   | 执行 SYSCALL 指令是否正确地产生系统调用例外               | P    | P       | ALL P -5 |
...
| 54   | SW                        | 结合 LW 指令，执行 SW 指令是否产生正确的访存结果             | P    | P       | -5       |
...
> 惊呆了！功能测例自己竟然破坏自己，使用一个ram，仿真结果46，板子47，实际应该4b？！